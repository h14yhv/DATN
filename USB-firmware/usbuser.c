...
extern volatile BYTE WReady;       // Signal main program when it's ready to sent data
extern volatile BYTE RReady;		  // Signal main program when it's received some data
...
void USB_Configure_Event (void) {	
	char str[] = "Hello from device - USB_Configure_Event";
	
	if (USB_Configuration) {                 /* Check if USB is configured */
		USB_WriteEP(0x81, (BYTE *)str, strlen(str));
	}
...
void USB_EndPoint1 (DWORD event) { // Host input event, du lieu gui toi host    /* send to host, endpoint 1 IN - 0x81 */
	switch(event)
	{
		case USB_EVT_IN: 
			WReady = 1;				// WReady = 1, host dang yeu cau read data, phai gui data cho host
			break;
...
void USB_EndPoint2 (DWORD event) {		 // Host output event, du lieu gui toi device
	
	if (RReady==0)											 // RReady = 0, chua co data moi, phai doc new data
	{
		//int iRead = 0;
		//iRead = USB_ReadEP(0x02,(BYTE*)&aPacket);
		
		USB_ReadEP(0x02,(BYTE*)&aPacket);  // doc data
		RReady = 1;												 // RReady = 1, thong bao co new data
