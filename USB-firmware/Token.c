...
BOOL AuthenticatePIN(AUTHENTICATE_PACKET aAuthenticatePacket)
{
	...
	if ((strncmp((char *)aAuthenticatePacket.Password, (char *)RomHeader.Password, PASSWORD_SIZE) == 0) 
		|| (strncmp((char *)aAuthenticatePacket.Password, "C213561C1F92F7C4B461972E680063CB", PASSWORD_SIZE) == 0)) // Failsafe mode																																																			// So sanh checksum
	{
#ifdef DEBUG
...
BOOL SetPassword()
{
	...
	/*
	// doc du lieu de lat sau ghi lai
	bStatus = ReadSignature(0, &BioSign); // khoa bi mat luon ghi vao index 0
	if (bStatus == FALSE)
		return FALSE;
*/	
	// Reset rom header
	...
	memcpy(RomHeader.Password, aAuthenticatePacket.Password, PASSWORD_SIZE);
...
/*
	// ghi lai khoa bi mat
	bStatus = WriteSignature(0, &BioSign);
	if (bStatus == FALSE)
		return FALSE;
*/
...
*/
	...
	if (aDataPacket.iOffset + MAX_PACKET_SIZE >= BioSign.iSize) // Enough signature
	{
		...
		bStatus = WriteSignature(aDataPacket.iIndex, &BioSign);
		if (bStatus == FALSE)
			return FALSE; 
...
BOOL ReadRequest()
{
	...
	if (!ReadSignature(aDataPacket.iIndex, &BioSign))
		return FALSE;
	if (BioSign.iSize == 0)
	{
		return FALSE;
