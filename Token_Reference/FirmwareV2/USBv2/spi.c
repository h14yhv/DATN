...
void EEPROM_Write(BYTE *srcbuf, DWORD length, WORD Addr)
{
	...
	do
	{
		...
		SPICmd[0] = WRITE;	    /* Write command is 0x02, low 256 bytes only */
	  	...
	  	SPICmd[1] = (Addr>>8)&0xFF;	
		SPICmd[2] = Addr&0xFF;
	  	
		AT91F_PIO_ClearOutput(AT91C_BASE_PIOA, SPI0_SEL);
		SPISend(SPICmd,3);
		SPISend(srcbuf+dwBytesWritten,(length-dwBytesWritten)>=64?64:length-dwBytesWritten);
	  	AT91F_PIO_SetOutput(AT91C_BASE_PIOA, SPI0_SEL);
		Addr+=64;
		
		dwBytesWritten+=(length-dwBytesWritten>=64)?64:length-dwBytesWritten;
...
void EEPROM_Read(BYTE *desbuf, DWORD length, WORD Addr)
{
	...
	cmdRead[0] = READ;		/* Read command is 0x03, low 256 bytes only */
  	//cmdRead[1] = Addr>>16;	
  	cmdRead[1] = (Addr>>8)&0xFF;
	cmdRead[2] = Addr&0xFF;
	//IOCLR0 = SPI0_SEL;
  	AT91F_PIO_ClearOutput(AT91C_BASE_PIOA, SPI0_SEL);
	SPISend( cmdRead, 3 );  //Gui lenh READ
  	SPIReceive( desbuf, length);
	AT91F_PIO_SetOutput(AT91C_BASE_PIOA, SPI0_SEL);
	
	//IOSET0 = SPI0_SEL;
	
	delay(10000); //Delay 250ns
